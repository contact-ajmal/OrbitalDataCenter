    ...
    <div className="pointer-events-none absolute inset-x-0 top-0 flex items-center justify-between px-4 py-3">
      ...
      <div className="flex items-center gap-3">
        <button
          ...
          className="pointer-events-auto flex h-6 w-6 items-center justify-center rounded-full border border-white/15 text-[11px] text-dim transition-colors hover:border-laser/60 hover:text-laser"
          title="Mission info"
        >
          ⓘ
        </button>
        <div className="flex items-center gap-2">
          ...
          <span className="text-[9px] uppercase tracking-[.22em] text-dim">Sim Active</span>
        </div>
        <span className="font-mono text-[11px] tabular-nums text-ink">{clock}</span>
      </div>
    </div>
  );
}
